...
describe("the GET /user route", () => {
    ...
    it("returns the user", async () => {
        ...
        const res2 = await app.get("/user")
            .set("Authorization", `Bearer ${token}`);
        expect(res2.status).toBe(HttpCode.OK);
        expect(res2.type).toBe("application/json");
        expect(res2.body.user.id).toBe(id);
        expect(res2.body.user.name).toBe(name);
        expect(res2.body.user.email).toBe(email);
        expect(res2.body.user.password).not.toBe(password);
    });
});
